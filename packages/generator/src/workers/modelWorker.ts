...
workerScope.onmessage = (event) => {
  ...
  if (!message) {
    workerScope.postMessage({
      version: 1,
      type: 'protocol-error',
      message: 'Unsupported model worker message',
    });
    return;
  }
  ...
  if (message.type === 'configure') {
    if (jobs.size > 0) {
      workerScope.postMessage({
        ...
        message: 'Cannot reconfigure while model work is running',
      });
      return;
    }
    try {
      broker = createModelBroker(
        message.config.manifest,
        createWebLLMAdapter(),
      ...
      workerScope.postMessage({
        ...
        message:
          error instanceof Error
            ? error.message
            : 'Model worker configuration failed',
      });
    }
    return;
  }
  if (!broker) {
    workerScope.postMessage({
      ...
      type: 'protocol-error',
      requestId: message.requestId,
      message: 'Configure the model worker before use',
    });
    return;
  }
  if (jobs.has(message.requestId)) {
    workerScope.postMessage({
      ...
      type: 'protocol-error',
      requestId: message.requestId,
      message: 'Model request is already running',
    });
    return;
  }
      ...
      : operation === 'load'
        ...
        : operation === 'generate-candidates'
          ...
          : operation === 'compose-clues'
            ? broker.composeClues(
                payload as Parameters<ModelBroker['composeClues']>[0],
                controller.signal,
              )
            : broker.unload();
  void result
