...
const request: FillRequest = {
  ...
  intersections: [],
  candidates: [
    { word: 'CAT', score: 1, lexemeId: 'cat', sourceIds: ['fixture'] },
  ],
};
...
describe('constructor worker client', () => {
  it('routes progress and resolves a worker result', async () => {
    ...
    if (!solve || solve.type !== 'solve')
      throw new Error('Expected solve request');
    ...
    worker.emit({
      ...
      result: {
        status: 'failed',
        failure: { code: 'unsatisfiable', message: 'no fill', nodes: 1 },
      },
    });
  ...
  it('sends cancellation and rejects malformed or stopped jobs', async () => {
    ...
    if (!malformedRequest || malformedRequest.type !== 'solve')
      throw new Error('Expected second solve request');
    worker.emit({
      version: 1,
      type: 'result',
      jobId: malformedRequest.jobId,
      result: { status: 'unknown' },
    });
    await expect(malformed).rejects.toThrow(
      'Invalid constructor worker response',
    );
