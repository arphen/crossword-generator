...
function batchCount(value: number | undefined): number | undefined {
  const count = value ?? 1;
  return Number.isInteger(count) && count >= 1 && count <= MAX_CANDIDATE_BATCHES
    ? count
    : undefined;
}
...
export async function generateCandidateBatches(
  ...
  for (let batch = 0; batch < count; batch += 1) {
    ...
    excludedAnswers.push(
      ...generated.value.map((suggestion) => suggestion.surface),
    );
  }
...
export async function constructOriginalFill(
  ...
  const generated = await generateCandidateBatches(
    broker,
    request.model,
    request.candidateBatches,
    signal,
  );
  if (!generated.ok)
    return { ok: false, error: { stage: 'model', ...generated.error } };
  ...
  const filled = await fillGrid.solve(
    { ...request.fill, candidates },
    { signal },
  );
  if (filled.status === 'solved' && filled.solution)
    return {
      ok: true,
  ...
  return {
    ...
    error: {
      ...
      message:
        failure?.message ??
        'The deterministic fill engine rejected the candidate set',
    },
  };
}
