...
describe('model worker protocol', () => {
  it('accepts configure, execution, and cancellation messages', () => {
    ...
    ).toMatchObject({ type: 'configure', requestId: 'one' });
    expect(
      parseModelWorkerRequest({
        version: 1,
        type: 'execute',
    ...
    ).toMatchObject({ type: 'execute', operation: 'load' });
    expect(
      parseModelWorkerRequest({ version: 1, type: 'cancel', requestId: 'two' }),
    ).toEqual({ version: 1, type: 'cancel', requestId: 'two' });
  });
  ...
  it('rejects invalid config, payloads, and results', () => {
    expect(
      parseModelWorkerRequest({
        ...
        config: {
          ...config,
          manifest: { ...config.manifest, distribution: 'remote-cloud' },
        },
      }),
    ).toBeUndefined();
    expect(
