...
function fakeModule(
  engine: WebLlmEngine,
  createSpy: (modelId: string) => Promise<WebLlmEngine>,
): WebLlmModule {
  return {
    prebuiltAppConfig: {
      model_list: [{ model_id: 'fixture-model' }, { model_id: 'other-model' }],
    },
...
describe('in-browser WebLLM adapter', () => {
  it('rejects model ids outside the prebuilt catalog without creating an engine', async () => {
    ...
    await expect(
      adapter.install({ ...manifest, id: 'unknown-model' }),
  ...
  it('normalizes answer fields and confidence labels from lightweight models', async () => {
    const engine = fakeEngine(
      '{"candidates":[{"answer":"coastal wetlands","intendedSense":"ecological feature","associations":["habitat",3],"role":"theme","confidence":"high"},{"answer":"mangroves","role":"interpreter","confidence":"low"}]}',
    );
    const adapter = createWebLLMAdapter({
      loadModule: async () => fakeModule(engine, async () => engine),
      createEngine: async () => engine,
  ...
  it('normalizes clue drafts to the broker mechanism vocabulary', async () => {
    const engine = fakeEngine(
      '[{"mechanism":"definition","text":"A watery expanse","difficulty":"easy"}]',
    );
    const adapter = createWebLLMAdapter({
      loadModule: async () => fakeModule(engine, async () => engine),
      createEngine: async () => engine,
    ...
    await expect(
      adapter.composeClues({
        answer: 'TIDALPOOL',
        intendedSense: 'a coastal body of water',
      }),
    ).resolves.toEqual([
