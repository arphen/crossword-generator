...
export type ModelWorkerOperation =
  | 'install'
  | 'load'
  | 'generate-candidates'
  | 'compose-clues'
  | 'unload';
type ModelWorkerPayload =
  | CandidateRequest
...
const modelStates: readonly ModelState[] = [
  'uninstalled',
  'installed',
  'loaded',
  'generating',
...
const operations: readonly ModelWorkerOperation[] = [
  'install',
  'load',
  'generate-candidates',
  'compose-clues',
...
function isManifest(value: unknown): value is ModelManifest {
  if (
    ...
    !isFiniteNumber(value.minimumMemoryMb) ||
    !Array.isArray(value.shards) ||
    (value.distribution !== undefined && value.distribution !== 'webllm-mlc')
  )
    return false;
...
function isRuntime(value: unknown): value is RuntimeProbe {
  return (
    ...
    isFiniteNumber(value.availableMemoryMb) &&
    isFiniteNumber(value.storageQuotaBytes) &&
    isFiniteNumber(value.storageUsageBytes)
  );
}
...
function isCandidateRequest(value: unknown): value is CandidateRequest {
  return (
    ...
    value.requestedRoles.every((role) =>
      ['theme', 'long', 'general', 'glue', 'stretch'].includes(role as string),
    ) &&
...
function isClueRequest(
  value: unknown,
): value is Readonly<{ answer: string; intendedSense: string }> {
  return (
    isRecord(value) &&
...
export function parseModelWorkerRequest(
  value: unknown,
): ModelWorkerRequest | undefined {
  ...
  if (
    ...
    isManifest(value.config.manifest) &&
    isRuntime(value.config.runtime)
  ) {
    return {
      version: 1,
      type: 'configure',
  ...
  if (
    ...
    typeof value.requestId !== 'string' ||
    !value.requestId ||
    !operations.includes(value.operation as ModelWorkerOperation)
  )
    return undefined;
  const operation = value.operation as ModelWorkerOperation;
  if (operation === 'generate-candidates' && !isCandidateRequest(value.payload))
    return undefined;
  if (operation === 'compose-clues' && !isClueRequest(value.payload))
    return undefined;
  return {
    version: 1,
    type: 'execute',
    requestId: value.requestId,
    operation,
    payload: value.payload as ModelWorkerPayload,
  };
}
...
export function parseModelWorkerResponse(
  ...
  if (
    ...
    (value.operation === 'configure' ||
      operations.includes(value.operation as ModelWorkerOperation)) &&
    isBrokerResult(value.result)
  ) {
    return {
      version: 1,
      type: 'result',
      requestId: value.requestId,
      operation: value.operation as ModelWorkerOperation | 'configure',
      result: value.result,
    };
  }
  return undefined;
