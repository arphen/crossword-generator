...
function isFillSlot(value: unknown): boolean {
  ...
  return (
    ...
    (typeof value.pattern === 'string' &&
      value.pattern.length === value.length &&
...
function isIntersection(value: unknown): boolean {
  if (
    ...
    typeof value.otherSlotId !== 'string'
  )
    return false;
  return (
    isNonNegativeInteger(value.position) &&
    isNonNegativeInteger(value.otherPosition)
  );
}
...
function isFillRequest(value: unknown): value is FillRequest {
  ...
  if (
    ...
    (typeof value.maxNodes !== 'number' ||
      !Number.isInteger(value.maxNodes) ||
      value.maxNodes < 1)
  )
    return false;
  if (
    value.qualityThreshold !== undefined &&
    !isFiniteNumber(value.qualityThreshold)
  )
    return false;
  return (
    value.excludedWords === undefined ||
    (Array.isArray(value.excludedWords) &&
      value.excludedWords.every((word) => typeof word === 'string'))
  );
}
...
export function parseConstructorWorkerRequest(
  value: unknown,
): ConstructorWorkerRequest | undefined {
  if (
    !isRecord(value) ||
    value.version !== 1 ||
  ...
  if (value.type === 'cancel')
    return { version: 1, type: 'cancel', jobId: value.jobId };
...
function isProgress(value: unknown): value is FillProgress {
  return (
    ...
    (isFiniteNumber(value.bestScore) ||
      value.bestScore === Number.NEGATIVE_INFINITY)
  );
}
...
function isResult(value: unknown): value is FillResult {
  if (
    !isRecord(value) ||
    !['solved', 'failed'].includes(value.status as string)
  )
    return false;
  ...
  if (
    ...
    ![
      ...
      'resource-limit',
      'invalid-request',
    ].includes(failure.code as string)
  )
    return false;
  return (
    typeof failure.message === 'string' && isNonNegativeInteger(failure.nodes)
  );
}

export function parseConstructorWorkerResponse(
  value: unknown,
): ConstructorWorkerResponse | undefined {
  ...
  if (value.type === 'progress' && isProgress(value.progress))
    return {
      version: 1,
      type: 'progress',
      jobId: value.jobId,
  ...
  if (value.type === 'result' && isResult(value.result))
    return {
      version: 1,
      type: 'result',
      jobId: value.jobId,
